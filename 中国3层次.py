...
def get_3type_df():
    ...
    # df=
    df=pd.DataFrame({})
    df["confirmed"] = pd.read_excel(state_filename_base.format("confirmed")).max()
    df["recovered"] = pd.read_excel(state_filename_base.format("recovered")).max()
    df["death"] = pd.read_excel(state_filename_base.format("death")).max()
    # pd 每一列 求和
    # df_max = df.max()
    #
    # df=pd.DataFrame({})
...
fig.update_layout(title="Distribution of Number of Active Cases 中国累计患病的分布(各个城市)",
                  xaxis_title="城市",yaxis_title="Number of Cases 患病的个数",)
